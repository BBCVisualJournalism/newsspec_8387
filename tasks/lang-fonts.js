module.exports = function (grunt) {
    ...
    grunt.registerTask('lang_font:default', function () {
        ...
        var config = grunt.file.readJSON('config.json'), service = config.services.default;
        ...
        var cssToPrependStr = grunt.file.read('source/scss/news_special/fonts/' + fontstackStr + '_fonts.css');

        for (var a = 0;  a < outputCssFilePaths.length; a++) {
            var sourceCssStr = grunt.file.read(outputCssFilePaths[a]);
    ...
    grunt.registerTask('lang_font:others', function () {
        ...
        var config = grunt.file.readJSON('config.json'), services = config.services.others;
